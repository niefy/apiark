...
export function ImportDialog({ open, onOpenChange }: ImportDialogProps) {
	...
	const [importUrl, setImportUrl] = useState(
		() => localStorage.getItem(LAST_IMPORT_URL_KEY) ?? "",
	);
	...
	const handleImport = useCallback(
		async (overwrite = false) => {
			...
			try {
				const collectionPath = await importCollection(
					filePath,
					format,
					targetDir,
					overwrite,
		...
		<div className="space-y-4">
			...
			{inputMode === "file" ? (
				<div className="space-y-2">
					...
					{filePath && (
						<p className="truncate text-xs text-[var(--color-text-dimmed)]">
							{filePath}
						</p>
					)}
				</div>
			...
			<div className="space-y-2">
				<label className="text-xs font-medium text-[var(--color-text-secondary)]">
					{t("import.format")}{" "}
					{format && (
						<span className="text-green-500">{t("import.autoDetect")}</span>
					)}
				</label>
		...
		<div className="space-y-4">
			<div>
				...
				<div className="mt-2 flex gap-4 text-xs text-[var(--color-text-secondary)]">
					<span>
						{preview.requestCount} request
						{preview.requestCount !== 1 ? "s" : ""}
					</span>
					<span>
						{preview.folderCount} folder{preview.folderCount !== 1 ? "s" : ""}
					</span>
					<span>
						{preview.environmentCount} environment
						{preview.environmentCount !== 1 ? "s" : ""}
					</span>
				</div>
			</div>
